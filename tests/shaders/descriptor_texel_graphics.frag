...
void main()
{
    uint packed_color = texelFetch(
        source_uint_buffers[parameters.uint_descriptor_index], 0).x;
    uint float_value = floatBitsToUint(texelFetch(
        source_float_buffers[parameters.float_descriptor_index], 0).x);

    if (vertex_values != uvec2(packed_color, float_value))
        packed_color = 0xff0000ffu;

    output_color = unpackUnorm4x8(packed_color);
}
